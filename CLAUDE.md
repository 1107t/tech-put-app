...
- **DRY原則**: 同じ処理が2ファイル以上に登場しそうになったら、カスタムフックや共通関数に切り出してから実装する。認証ガード・データ取得・共通ロジックは特に注意
- **単一責任原則**: コンポーネントにデータ定義（メニュー配列・設定値など）を直書きしない。データは `src/lib/` 以下の専用ファイルに定義し、コンポーネントはpropsで受け取るだけにする
- **新規ページ作成時**: 既存ページとの共通処理を事前に洗い出し、重複が生じない設計にしてから実装を始める
...
## Scope
- Working directory: /Users/TatsuyaUchida/tech-put-app
